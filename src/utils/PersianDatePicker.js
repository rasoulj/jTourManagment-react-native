...
export default class PersianDatePicker extends Component {
    ...
    render() {
        ...
        return (
            <TouchableWithoutFeedback onPress={()=> {if(!this.props.disabled) Picker.show()}} >
    ...
    _createDates(){
        ...
        if(this.props.maxDate==undefined){
            ...
            var m =moment(this.props.maxDate,'jYYYY/jM/jD');
            var MaxMonth=m.jMonth();
            var MaxYear=m.jYear();
            var MaxDay=m.jDate();
        }
        var m =this.props.minDate==undefined ? moment() : moment(this.props.minDate,'jYYYY/jM/jD');
        var month=m.jMonth();
        var year=m.jYear();
        var day=m.jDate();
